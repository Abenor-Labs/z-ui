import type { ReactNode } from 'react';
import { NavLink, Link, useLocation } from 'react-router-dom';
import { LogoMark } from './LogoMark';
import { REPO_URL, NPM_URL } from '../data/registry';

const NAV = [
  { to: '/components', label: 'components' },
  { to: '/lab', label: 'lab' },
  { to: '/docs', label: 'docs' },
];

export function Shell({ children }: { children: ReactNode }) {
  /**
   * The landing page has no footer.
   *
   * It is a header and one hero, and the footer repeats things it already
   * says — the tagline, the version, the licence — under a page that is
   * already only two screens tall. Every other route earns it: they are long,
   * and a reader who reaches the bottom of /docs or a component page needs
   * somewhere to go next. The hero's own CTA is that somewhere here.
   */
  const landing = useLocation().pathname === '/';

  return (
    <div className="shell">
      <header className="topbar hairline-b">
        <div className="frame topbar-inner">
          <Link to="/" className="wordmark" aria-label="Z-UI home">
            <LogoMark />
          </Link>
          <nav className="nav mono" aria-label="Main">
            {NAV.map((n) => (
              <NavLink
                key={n.to}
                to={n.to}
                className={({ isActive }) => `nav-link${isActive ? ' nav-active' : ''}`}
              >
                {n.label}
              </NavLink>
            ))}
            {/* An ask, not a metric. The count is deliberately absent — a badge
                reading 0 argues against itself, and the number can go in the
                day there is one worth printing. Mark plus label reads as
                "star on GitHub" without needing either word. */}
            <a
              href={REPO_URL}
              target="_blank"
              rel="noreferrer"
              className="nav-star"
              aria-label="Star Z-UI on GitHub"
            >
              <svg
                viewBox="0 0 16 16"
                width="14"
                height="14"
                fill="currentColor"
                aria-hidden="true"
              >
                <path d="M8 0c4.42 0 8 3.58 8 8a8.013 8.013 0 0 1-5.45 7.59c-.4.08-.55-.17-.55-.38 0-.27.01-1.13.01-2.2 0-.75-.25-1.23-.54-1.48 1.78-.2 3.65-.88 3.65-3.95 0-.88-.31-1.59-.82-2.15.08-.2.36-1.02-.08-2.12 0 0-.67-.22-2.2.82-.64-.18-1.32-.27-2-.27-.68 0-1.36.09-2 .27-1.53-1.03-2.2-.82-2.2-.82-.44 1.1-.16 1.92-.08 2.12-.51.56-.82 1.28-.82 2.15 0 3.06 1.86 3.75 3.64 3.95-.23.2-.44.55-.51 1.07-.46.21-1.61.55-2.33-.66-.15-.24-.6-.83-1.23-.82-.67.01-.27.38.01.53.34.19.73.9.82 1.13.16.45.68 1.31 2.69.94 0 .67.01 1.3.01 1.49 0 .21-.15.45-.55.38A7.995 7.995 0 0 1 0 8c0-4.42 3.58-8 8-8Z" />
              </svg>
              star
            </a>
          </nav>
        </div>
      </header>

      <div className="frame ruled main-frame">{children}</div>

      {landing ? null : (
        <footer className="footer hairline-t">
          <div className="frame footer-inner">
            <div className="footer-col footer-brand">
              <span className="footer-mark">Z-UI</span>
              <span className="mono footer-note">
                Micro-animations you own. v0.1 — early; names and props may change before v1.
                Registry served from raw GitHub.
              </span>
            </div>
            <div className="footer-col mono footer-links">
              <a href={REPO_URL} target="_blank" rel="noreferrer">
                github.com/Abenor-Labs/z-ui
              </a>
              <a href={NPM_URL} target="_blank" rel="noreferrer">
                npm: @abenor/z-ui
              </a>
              <a href={`${REPO_URL}/issues`} target="_blank" rel="noreferrer">
                issues
              </a>
            </div>
            <div className="footer-col mono footer-license">MIT © Abenor Labs</div>
          </div>
        </footer>
      )}
    </div>
  );
}
